...
class Book24Spider(scrapy.Spider):
    ...
    def parse_link(self, response):
        ...
        book_data = response.css("dl.product-characteristic__list")
        ...
        if author_el.css("span.product-characteristic__label::text").get() == " Автор: ":
            result["authors"] = author_el.xpath("//div/dt/span[contains(text(), ' Автор: ')]/../../dd[@class='product-characteristic__value']/a[@class='product-characteristic-link smartLink']/text()").get()
            if result["authors"] == None:
                result["authors"] = author_el.xpath(
                    "//div/dt/span[contains(text(), ' Автор: ')]/../../dd[@class='product-characteristic__value']/text()").get()
        ...
        result["rating"] = response.xpath(
            "//div/dt/span[contains(text(), ' Возрастное ограничение: ')]/../../dd[@class='product-characteristic__value']//text()"
        ).get()
